...
def etrace_run_loop(env, agent, title, max_e=None):
    t = 0; i = 0; e = 0
    s, r, d, _ = env.reset()
    agent.reset()
    a_ = agent.action(s)
...
for i in range(num_runs):
    NSARSA_Learning = TrueOnlineLambdaSARSA(env.obspace_shape(), env.nactions(), n=n)
    ep_lens, rewards = etrace_run_loop(env, NSARSA_Learning, 'NSARSALearning, n='+str(n), max_e=eps_per_run)
